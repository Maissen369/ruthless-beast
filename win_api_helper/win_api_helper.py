# win_api_helper/win_api_helper.py
...
# TODO: Might want to reimplement the toolhelp snapshot functions as their unicode counterparts
...
# These two functions are set up below.
...
def _psapifunc(fname):
    global _exdll
    try:
        return getattr(_exdll, fname)
    except AttributeError:
        try:
            return getattr(_exdll, 'K32' + fname)
        except AttributeError:
            if 'psapi' in _exdll._name:
                raise
...
def ReadProcessMemory(size, base=None, handle=None):
    ...
    elif not isinstance(base, c_void_p):
...
def limits(typ):
    ...
    min_val = (max_val * -1)
    umax -= 1
    return min_val, max_val, umin, umax
...
##############################
# Define the call_zpoint_function
##############################

def call_zpoint_function(dll_path):
    """Calls the Zpoint function from a provided DLL path."""
    # Load the DLL
    try:
    ...
    # Access the exported Zpoint function
    try:
        zpoint_func = my_dll.Zpoint
    except AttributeError:
        print(f"Failed to locate 'Zpoint' function in the DLL '{dll_path}'")
        return

    # Define Zpoint function signature (int Zpoint())
    zpoint_func.restype = c_int  # Zpoint returns an integer

    # Call the function
    try:
        result = zpoint_func()
    except Exception as e:
        print(f"Exception occurred while calling 'Zpoint': {e}")
        return

    # Display result
    if result == 0:
        print("[+] Zpoint executed successfully.")
    else:
        print(f"[!] Zpoint execution failed with error code: {result}")
